...
async function run() {
  ...
  // 2. Clear old Master Data and Parks to ensure clean 5-Corp structure
  await Corporation.deleteMany({ districtId: dist._id });
  await Zone.deleteMany({ districtId: dist._id });
  await Ward.deleteMany({ districtId: dist._id });
  await Park.deleteMany({ district: dist._id });
  console.log('Cleared existing master data and parks for Bangalore Urban');

  // 3. Create the 5 Corporations
  const corpMapping = {
    ...
    'Bengaluru West City Corporation': ['West', 'R.R. Nagar', 'Rajarajeshwari Nagar', 'Chandra Layout']
  };

  const createdCorps = {};
  for (const corpName of Object.keys(corpMapping)) {
    createdCorps[corpName] = await Corporation.create({ 
      name: corpName, 
      districtId: dist._id, 
      code: corpName.substring(0, 5).toUpperCase().replace(/\s/g, '') 
    });
    console.log(`Created Corporation: ${corpName}`);
  }

  // Helper to determine Corporation based on old zone/ward name
  const getCorpForPark = (zoneName, wardName) => {
    for (const [corpName, keywords] of Object.entries(corpMapping)) {
      if (keywords.some(k => zoneName.includes(k) || wardName.includes(k))) {
        return createdCorps[corpName];
      }
    }
    // Default fallback
    return createdCorps['Bengaluru Central City Corporation'];
  };

  // 4. For each park, find or create zone + ward under the new Corporation, then create park
  let successCount = 0;
  for (const p of parksData) {
    try {
      const targetCorp = getCorpForPark(p.zoneName, p.wardName);
      
      // Ensure Zone name follows the "2 Zones per Corp" rule (Zone 1 / Zone 2)
      // For simplicity, we just create a Zone named after the old zone, but nested in the 5 Corp
      let zone = await Zone.findOne({ name: p.zoneName, corporationId: targetCorp._id });
      if (!zone) {
        zone = await Zone.create({ name: p.zoneName, code: p.zoneName.substring(0,3).toUpperCase(), corporationId: targetCorp._id, districtId: dist._id });
      }

      // Find or create ward
      let ward = await Ward.findOne({ name: p.wardName, zoneId: zone._id });
      if (!ward) {
        // Generate random ward number if not present
        const wnum = Math.floor(Math.random() * 200) + 1;
        ward = await Ward.create({ name: p.wardName, wardNumber: wnum.toString(), zoneId: zone._id, corporationId: targetCorp._id, districtId: dist._id });
      }
