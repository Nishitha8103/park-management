...
const AdminInspections = () => {
  ...
  const filteredComplaints = complaints.filter(c => {
    const isInspectionRelated = ['Completed - Waiting for Admin Review', 'Inspection Pending', 'Verified', 'Closed', 'Returned by Admin', 'Rework Required'].includes(c.status);
    if (!isInspectionRelated) return false;
  ...
  return (
    <div className="admin-panel" style={{ padding: '1rem' }}>
      <div className="admin-panel-header" style={{ marginBottom: '1.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          <h2 style={{ color: '#0f2d52', margin: 0, fontWeight: 700 }}>Inspection Review</h2>
          <p style={{ color: '#64748b', margin: '4px 0 0 0' }}>Review inspections submitted by government officials</p>
        </div>
      </div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '0.9rem' }}>
              <thead>
                <tr style={{ borderBottom: '2px solid #e2e8f0', color: '#475569' }}>
                  ...
                  <th style={{ padding: '10px' }}>Inspected By</th>
                  <th style={{ padding: '10px' }}>Inspection Date</th>
                ...
                {filteredComplaints.map(c => {
                  ...
                  return (
                    <tr key={c._id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                      ...
                      <td style={{ padding: '12px 10px' }}>
                        {c.assignedOfficial?.name || <span style={{ color: '#94a3b8', fontStyle: 'italic' }}>Pending Inspector</span>}
                      </td>
                      <td style={{ padding: '12px 10px' }}>
                        {c.inspectionDate ? new Date(c.inspectionDate).toLocaleDateString('en-IN') : <span style={{ color: '#94a3b8' }}>—</span>}
                      </td>
                      <td style={{ padding: '12px 10px' }}>
                        {c.inspectionCondition ? (
                          <span style={{ fontWeight: 800, color: '#7c3aed' }}>{c.inspectionCondition}</span>
                      ...
                      <td style={{ padding: '12px 10px' }}>
                        <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                          {/* Removed Assign Contractor button, focusing on Inspections */}
                          ...
                          <button
                            ...
                            style={{ background: '#ef4444', color: '#fff', border: 'none', padding: '6px 10px', borderRadius: '6px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
